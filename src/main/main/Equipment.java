...
public class Equipment {
	...
	private boolean swap(int value){
		ArrayList<String> equip1 = this.fetch();
		this.setEquipID(Integer.parseInt(equip1.get(0)));
		this.setEquipName(equip1.get(1));
		this.setEquipDesc(equip1.get(2));
		this.setEquipCap(Integer.parseInt(equip1.get(3)));
		
		//get second equip data
		this.setEquipCap(value);
		ArrayList<String> equip2 = this.fetch();
		
		//set equip id back but with new name from equip data
		this.setEquipID(Integer.parseInt(equip1.get(0)));
		this.setEquipName(equip2.get(1));
		dbInstance.startTrans();
		boolean res = this.post();
		if(res == false){
			dbInstance.rollbackTrans();
			return false;
		}
		this.setEquipID(Integer.parseInt(equip2.get(0)));
		this.setEquipName(equip1.get(1));
		this.setEquipDesc(equip2.get(2));
		this.setEquipCap(Integer.parseInt(equip2.get(3)));
		res = this.post();
		if(res == false){
			dbInstance.rollbackTrans();
			return false;
	...
	public static void main(String[] args){
		...
		if(result){
			...
			eq.setEquipName(res.get(1));
			eq.setEquipDesc(res.get(2));
			eq.setEquipCap(Integer.parseInt(res.get(3)));
			boolean succ = eq.swap(22);
						
			eq.printTable();
			
			if(!succ){
				System.out.println("Failed, transaction rolled back.");
			}else{
				System.out.println("Failed, transaction rolled back.");
			}
		}
